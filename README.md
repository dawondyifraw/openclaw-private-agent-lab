...
## 1. Overview
This system is a multi-agent orchestration environment built on OpenClaw. It serves as a personal assistance home lab for agentic experimentation, automated translation routing, and mixed cloud/local model execution.
...
## Starting Point / Bootstrap (Live Root + Archive Policy)

- **Live root (only runtime):** `/home/devbox/.openclaw`
- **Cold archives base:** `/home/devbox/archives/`
- **Archive naming:** `openclaw_legacy_<YYYY-MM-DD>`
- **Policy:** legacy copies are moved into the cold archive, secrets in archived copies are redacted, and archives are made read-only.

### Tracked vs Runtime State (Read This First)

Tracked in git (reviewable):
- Code, tests, documentation
- Policy-as-code and schemas
- `.example` templates only (never real keys/tokens)

Runtime-only (never committed):
- `.env` (API keys, tokens)
- `openclaw.json` (real runtime config)
- Telegram offsets (`telegram/update-offset*.json`)
- Agent auth stores (`**/agent/auth-profiles.json`)
- Sessions, memory DBs, workspaces, logs

### Bootstrap (Minimal)

Bootstrap sequence (from the live root):
```bash
cd /home/devbox/.openclaw

# Create runtime config from templates (do not commit)
...
# Run tests before starting systemd
# Recommended pre-flight (strict): fails on missing prerequisites
OPENCLAW_TEST_MODE=strict bash tests/run_all_tests.sh

# Start the gateway only after tests
systemctl --user start openclaw-gateway.service
```

Proof checks (should reference the live root only):
```bash
systemctl --user cat openclaw-gateway.service
docker compose -f /home/devbox/.openclaw/docker-compose.yml config --services
```

### Canonical Workspace Root

- Canonical workspace root is **plural**: `/home/devbox/.openclaw/workspaces`
- Compatibility path: `/home/devbox/.openclaw/workspace` should resolve to the canonical root.
- Legacy workspace backups must not live under the live root; they belong under `/home/devbox/archives/`.
...
Workspace rule: `workspaces/` is runtime scratch only. Docs/policies live under `docs/` and `agents/`.

### Why Silence Is A Feature

No response is often intentional:
- Mention-only policy (main) or group allowlist/bindings prevented a reply.
- Provider auth missing/invalid (strict tests will show which provider failed).
- Sandbox auth missing (runner rejects unauthenticated calls by design).
- A message was deleted/ignored due to isolation rules.
...
## 4. Model Strategy
All agents are Gemini-first by default unless explicitly overridden per-agent.
`qwen2.5:14b` is locked as the final fallback.

- **Primary default**: Google Gemini (`gemini-2.5-flash`)
- **Fallback 1**: OpenRouter (`google/gemini-2.0-flash-001`)
- **Fallback 2**: Groq (`llama-3.3-70b-versatile`)
- **Fallback 3 / locked last**: Ollama (`qwen2.5:14b`)

Assistant is intentionally overridden to use Mistral primary (`mistral-large-2411`) with local Ollama fallbacks.

All configured models remain available for manual `/model` switching. Automatic failover uses the configured fallback chain and emits a switch notice.
...
Pull + verify required local models:
```bash
bash /home/devbox/.openclaw/scripts/pull_ollama_models.sh
```

Confirm local inventory:
```bash
ollama list
```

Mistral cloud access requires `MISTRAL_API_KEY` in `.env` (never print the key).
...
- `/dash` or `/dashboard`: Show a summary of active tasks and reminders.
...
- **Sandbox Runner**: Verifies tool-runner sandbox health, workspace containment, and network egress rules.
...
## Performance & Stability (Low-Risk Tuning)

- Canonical guide: `docs/PERF.md`
- Benchmark script: `scripts/bench_ollama.sh`
- Conservative Ollama knobs are template-driven in `.env.example`:
  - `OLLAMA_NUM_PARALLEL`
  - `OLLAMA_MAX_LOADED_MODELS`
  - `OLLAMA_KEEP_ALIVE`
- Compaction-safe baseline is documented in `openclaw.json.example` under:
...
```bash
cd /home/devbox/.openclaw
bash scripts/bench_ollama.sh
```

Auto preflight before gateway start:
```bash
cd /home/devbox/.openclaw
...
## 14. Sandbox Tool Runner
The sandbox consists of a host-exposed guard plus an internal tool runner that spawns per-invocation job containers for **Tier 1** tool execution (workspace-scoped file ops and a strict allowlist of commands).

Details and threat model: `docs/TOOL_RUNNER_SANDBOX.md`

- **Endpoint**: `http://localhost:18888`
- **Health**: `GET /health`
- **Run**: `POST /run`
- **Workspaces (host)**: `~/.openclaw/workspaces/<agent>/group_<chat_id>/`
- **No Secrets Mounts**: `~/.openclaw/secrets` is never mounted into the sandbox.
- **No Internet Egress**: The sandbox runs on an internal-only Docker network; only internal allowlisted service calls are permitted by policy.

OpenClaw tool definition: `tools/sandbox_tool_run.json`
...
## 15. Telegram Sanitizer Patch Note
Runtime Telegram sanitizer/command-guard patch provenance is documented in:

- `docs/TELEGRAM_SANITIZER_PATCH.md`

This includes marker names, enforced behavior (unknown slash static handling, diagnostics suppression), and verification steps (`run_all_tests.sh` + Telegram smoke checks).

## Telegram Command Contract

Telegram slash commands are **default-deny**.

- Unknown `/...` commands return static `Unknown command. Use /help.` and **must not hit the LLM**.
- Dashboard commands are allowlisted only in the assistant dashboard group (see `docs/TESTING.md`).
