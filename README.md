...
- **Architecture**:
  ...
  - **Embeddings**: Local Ollama service using `nomic-embed-text`.
  - **API Service**: `rag-service` (FastAPI) handles ingestion and querying.
...
#### Querying
The `rag-service` exposes a simple API for querying:
```bash
curl -X POST http://localhost:8811/query \
     -d '{"query": "What is the security policy?", "top_k": 3}'
```

#### Rebuilding the Index
The index is persistent in the `chroma_data` volume. To force a full re-index:
1. Stop the services: `docker compose down`
2. Remove the volume: `docker volume rm openclaw_chroma_data`
