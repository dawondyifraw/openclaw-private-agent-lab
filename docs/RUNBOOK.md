...
5.  **Run E2E Verification**:
    ...
    OPENCLAW_TEST_MODE=strict bash ~/.openclaw/tests/run_all_tests.sh
    ```
...
### Workspace Root Confusion (workspace vs workspaces)
- **Canonical**: `~/.openclaw/workspaces` (plural)
- **Compatibility**: `~/.openclaw/workspace` should resolve to `~/.openclaw/workspaces`
- **Verify**: `readlink -f ~/.openclaw/workspace`
