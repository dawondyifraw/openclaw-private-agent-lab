...
These variables affect Telegram diagnostics, and are intended to be used only for the owner DM.

- `TELEGRAM_DEBUG`:
  - `false` (default): suppresses diagnostic details in Telegram responses.
  - `true`: may include slightly more detail for the owner DM only.
- `OPENCLAW_OWNER_TELEGRAM_ID`:
  - Telegram chat id for the owner DM (used to scope debug-only responses).
- `OPENCLAW_ASSISTANT_DASH_GROUP_ID`:
  - Telegram group id that is allowed to use dashboard commands (`/dash`, `/task`, `/remind`).

### How To Add A New Command Safely

1. Add it to the Telegram known-command allowlist injection in:
   - `scripts/patch_telegram_command_allowlist.sh` (marker: `OPENCLAW_TELEGRAM_COMMAND_ALLOWLIST_V1`)
2. Add/extend strict tests to lock the contract:
...
This repo intentionally keeps runtime secrets/state out of git.
