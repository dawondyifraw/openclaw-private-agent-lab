...
- Failover behavior is exercised by strict tests:
  - `tests/test_main_failover.sh` (OpenRouter -> Groq path).
  - `tests/test_fallback_under_cooldown.sh` (Groq failure path recovers via Gemini/OpenRouter).
...
- Strict requires Google Gemini + OpenRouter + Groq to be working (live provider probes).
- Kimi and Minimax are currently disabled in default chains and reported as `EXPECTED_FAIL` (non-blocking).
