...
if [ -z "$GOOGLE_API_KEY" ]; then
    # Google (Gemini direct) is optional for STRICT: OpenRouter+Groq are the required providers.
    if [ "${OPENCLAW_TEST_MODE:-default}" = "strict" ]; then
        echo -e "${YELLOW}EXPECTED_FAIL${NC} (Key not found in .env; Gemini not required for strict)"
        exit 0
    fi
...
if [ "$STATUS_CODE" == "200" ]; then
    ...
    if [ "${OPENCLAW_TEST_MODE:-default}" = "strict" ]; then
        echo -e "${YELLOW}EXPECTED_FAIL${NC} (HTTP $STATUS_CODE; Gemini currently failing and is not required for strict)"
        exit 0
    fi
