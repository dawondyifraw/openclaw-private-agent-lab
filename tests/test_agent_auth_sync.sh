...
for agent in "${AGENTS[@]}"; do
    ...
    if [ -f "$FILE" ]; then
        ...
        # Check if ollama:default exists with a local-safe auth type.
        if jq -e '.profiles["ollama:default"].provider == "ollama" and (.profiles["ollama:default"].type == "api_key" or .profiles["ollama:default"].type == "none")' "$FILE" > /dev/null 2>&1; then
             echo "    [Ollama Auth: present]"
        else
