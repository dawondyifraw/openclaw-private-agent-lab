...
# If the config doesn't include telegram channel configuration, skip (this repo may use plugin defaults).
if ! jq -e '.channels.telegram? // empty' "$CONFIG_FILE" >/dev/null 2>&1; then
    if [ "${OPENCLAW_TEST_MODE:-default}" = "strict" ]; then
        echo "FAIL: openclaw.json does not define .channels.telegram (strict mode)."
        exit 1
    fi
    echo "SKIP: openclaw.json does not define .channels.telegram; telegram config test not applicable."
    exit 0
fi
...
EXPECTED_BINDINGS["TG_GROUP_ASSISTANT_DASHBOARD_ID"]="assistant"
EXPECTED_BINDINGS["TG_OWNER_ID"]="main"

echo "=== Test 1: requireMention=false for agent-bound groups (always-on) ==="
for group in "${EXPECTED_GROUPS[@]}"; do
    require_mention=$(jq -r ".channels.telegram.groups.\"$group\".requireMention // false" "$CONFIG_FILE")
    
    if [ "$require_mention" = "false" ]; then
        echo "✓ PASS: Group $group has requireMention=false"
        ((PASS++))
    else
        echo "❌ FAIL: Group $group has requireMention=$require_mention (expected false)"
        ((FAIL++))
    fi
done
echo
...
if ! jq -e '.bindings? // empty' "$CONFIG_FILE" >/dev/null 2>&1; then
    if [ "${OPENCLAW_TEST_MODE:-default}" = "strict" ]; then
        echo "FAIL: openclaw.json does not define .bindings (strict mode)."
        exit 1
    fi
    echo "SKIP: openclaw.json does not define .bindings; binding test not applicable."
    exit 0
fi
for group in "${EXPECTED_GROUPS[@]}"; do
...
echo -n "Checking owner DM binding (TG_OWNER_ID -> main)... "
owner_agent=$(jq -r ".bindings[] | select(.match.channel == \"telegram\" and .match.peer.id == \"TG_OWNER_ID\") | .agentId" "$CONFIG_FILE")
if [ "$owner_agent" = "main" ]; then
    echo "✓ PASS"
    ((PASS++))
else
    echo "❌ FAIL (got '$owner_agent')"
    ((FAIL++))
fi
echo
