...
# - Required: Google + OpenRouter + Groq (all must be configured).
# - Optional/non-blocking: Kimi, Minimax (currently disabled/failing).
if [ "$MODE" = "strict" ]; then
    REQUIRED_VARS=("GOOGLE_API_KEY" "OPENROUTER_API_KEY" "GROQ_API_KEY" "OPENCLAW_GATEWAY_TOKEN")
else
    REQUIRED_VARS=("GOOGLE_API_KEY" "GROQ_API_KEY" "OPENROUTER_API_KEY")
fi
