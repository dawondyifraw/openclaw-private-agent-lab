...
PATTERN='(400|401|403) status code|timeout|Gateway timeout|no body'
LOG_HITS="$(journalctl --user -u "${UNIT}" --since "${SINCE}" --no-pager | rg -n -i "${PATTERN}" || true)"
if [[ -n "${LOG_HITS}" ]]; then
  echo "FAIL: raw HTTP/provider error leakage found in gateway logs since ${SINCE}"
...
pat = re.compile(r"(400|401|403) status code|timeout|Gateway timeout|no body", re.I)
...
if [[ -n "${SESSION_HITS}" ]]; then
  echo "FAIL: raw HTTP/provider error leakage found in Telegram session logs since ${SINCE}"
  echo "${SESSION_HITS}" | sed -n '1,40p'
  exit 1
fi

echo "PASS: no raw HTTP/provider error leakage found since ${SINCE}"
