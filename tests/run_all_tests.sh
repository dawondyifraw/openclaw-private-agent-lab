...
run_test() {
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        if echo "$OUTPUT" | rg -q "EXPECTED_FAIL"; then
            echo -e "${RED}FAIL${NC} (unexpected EXPECTED_FAIL marker)"
            RESULTS_TABLE+="| $name | FAIL | Unexpected EXPECTED_FAIL marker |"$'\n'
...
# 3.2 Google/OpenAI-compat transient failover patch presence (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_google_compat_failover_patch.sh" ]; then
    run_test "Google Compat Failover Patch" "${BASE_DIR}/tests/test_google_compat_failover_patch.sh"
fi

# 3.3 Telegram reply-wrapper sanitizer presence (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_reply_wrapper_sanitizer_present.sh" ]; then
    run_test "Telegram Reply Wrapper Sanitizer" "${BASE_DIR}/tests/test_telegram_reply_wrapper_sanitizer_present.sh"
fi

# 3.4 Telegram known commands allowlist (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_known_commands.sh" ]; then
    run_test "Telegram Known Commands" "${BASE_DIR}/tests/test_telegram_known_commands.sh"
fi

# 3.5 Telegram unknown commands remain static (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_unknown_commands_static.sh" ]; then
    run_test "Telegram Unknown Commands Static" "${BASE_DIR}/tests/test_telegram_unknown_commands_static.sh"
fi

# 3.6 Telegram tool-intent relay to main (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_tool_intent_main_relay.sh" ]; then
    run_test "Telegram Tool Intent Relay" "${BASE_DIR}/tests/test_telegram_tool_intent_main_relay.sh"
fi

# 4. Agent Auth Sync
run_test "Agent Auth Sync" "${BASE_DIR}/tests/test_agent_auth_sync.sh"

# 4.1 Agent Model Sync
if [ -f "${BASE_DIR}/tests/test_agent_models_sync.sh" ]; then
    run_test "Agent Model Sync" "${BASE_DIR}/tests/test_agent_models_sync.sh"
fi
...
# 11.3 Fallback Locking (strict only)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_fallback_locking.sh" ]; then
    run_test "Fallback Locking" "${BASE_DIR}/tests/test_fallback_locking.sh"
fi
...
# 14.1b Forbidden tool calls in main Telegram sessions (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_main_forbidden_toolcalls.sh" ]; then
    run_test "Main Forbidden ToolCalls" "${BASE_DIR}/tests/test_main_forbidden_toolcalls.sh"
fi
...
# 14.5 Telegram no heartbeat/cron diagnostic leakage (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_no_heartbeat_leaks.sh" ]; then
    run_test "Telegram No Heartbeat Leaks" "${BASE_DIR}/tests/test_telegram_no_heartbeat_leaks.sh"
fi

# 14.6 Telegram no raw HTTP/provider error leakage (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_telegram_no_raw_http_error_leaks.sh" ]; then
    run_test "Telegram No Raw HTTP Error Leaks" "${BASE_DIR}/tests/test_telegram_no_raw_http_error_leaks.sh"
fi

# 14.7 Commands No-LLM Guard (strict)
...
# 14.8 Provider guard for /model(s) switches (strict)
...
# 14.9 No disallowed tool calls in non-main Telegram sessions (strict)
...
# 14.10 Telegram language policy (strict, best-effort when recent traffic exists)
...
# 16. Commit hygiene guard (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_approved_files_guard.sh" ]; then
    run_test "Approved Files Guard" "${BASE_DIR}/tests/test_approved_files_guard.sh"
fi
