...
# 4.1 Provider Auth Propagation (strict)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_provider_auth_propagation.sh" ]; then
    run_test "Provider Auth Propagation" "${BASE_DIR}/tests/test_provider_auth_propagation.sh"
fi

# 4.2 /models Safety (strict, config-level)
if [ "${OPENCLAW_TEST_MODE}" = "strict" ] && [ -f "${BASE_DIR}/tests/test_models_command_safety.sh" ]; then
    run_test "Models Command Safety" "${BASE_DIR}/tests/test_models_command_safety.sh"
fi
...
# 11.2 Fallback Under Cooldown (strict)
